...
Fifteen case studies across three tracks, built around that.
...
| `utilities.css` | The 153 Tailwind utilities the VIO page uses, replacing the Play CDN so nothing compiles in the browser |
| `tour.js` | The guided-walkthrough engine, shared by fourteen case studies. VIO runs its own inline |
| `favicon.svg` | Hexagon and trace, drawn for 16px rather than scaled down from the masthead mark |
| `og-preview.png` | 1200×630 link-preview image referenced by the Open Graph tags |
| `headshot.jpg` | Home page rail |
| `Stethem_Austin_Resume.pdf` | Linked from the home page rail and footer |
| `CLAUDE.md` | Working notes: token architecture, voice, disclosure rules, and the failure modes that have already caught someone out |
| `.gitignore` | Blocks the private working documents by name. They hold the unrounded figures and must not be committed |
...
Every figure in these dashboards is synthetic and generated, not anonymised. That distinction is deliberate: anonymised records can be re-identified, and generated ones cannot. Branch coordinates, landed costs, POS revenue and margin targets are all produced rather than masked.

The architecture, the elasticity logic and the statistical methods are the production ones. Findings described in prose are real observations, stated as ratios or rounded magnitudes: no price, cost, margin, discount divisor, customer name, account identifier or vendor term appears anywhere in this repository.
