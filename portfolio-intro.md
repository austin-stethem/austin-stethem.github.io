...
A note on what you're looking at: the production versions of these engines ran as a
Flask application serving model output internally, against live ERP data. What's
published here is a static reimplementation of the same logic against an anonymized
proxy dataset — a server wired to a live cost table isn't something I can put on the
public internet.
